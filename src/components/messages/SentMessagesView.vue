...
<template>
    <div class="min-h-screen bg-gray-50 p-6 md:p-8">
                ...
                <table class="w-full text-left">
                    <thead>
                        <tr class="border-b border-gray-100 text-gray-600">
                            <th class="py-3 px-4 font-medium text-sm">Recipient</th>
                            <th class="py-3 px-4 font-medium text-sm">Message</th>
                            <th class="py-3 px-4 font-medium text-sm">Sent At</th>
                            <th class="py-3 px-4 font-medium text-sm">Status</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="message in messages" :key="message.id" class="border-b border-gray-100 hover:bg-gray-50 transition-colors duration-200">
                            <td class="py-3 px-4 text-gray-800">{{ message.recipient }}</td>
                            <td class="py-3 px-4 text-gray-800">{{ message.message }}</td>
                            <td class="py-3 px-4 text-gray-800">{{ new Date(message.sent_at).toLocaleString() }}</td>
                            <td class="py-3 px-4">
                                <span :class="{
                        ...
                        <tr v-if="messages.length === 0">
                            <td colspan="4" class="py-6 text-center text-gray-500 text-sm">No messages found</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <!-- Error Message -->
        <div v-if="errorMessage" class="mt-6 text-red-600 text-sm font-medium text-center">{{ errorMessage }}</div>
    </div>
</template>
